...
import { FilterSettings } from '../../shared/types'
...
interface FilterProps {
    show: boolean
    initial: FilterSettings | undefined
    onChange: (settings: FilterSettings | undefined) => void
    onClose: () => void
}
...
export default function Filter({ show, initial, onChange, onClose }: FilterProps) {
    ...
    const update = (settings: Partial<FilterSettings>) => {
        const newValue = {...value, ...settings} as FilterSettings
        setValue(newValue)
        input.current?.focus()
